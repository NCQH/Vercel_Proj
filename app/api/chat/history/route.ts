...
export async function GET(request: Request) {
  try {
    ...
    const limit = incomingUrl.searchParams.get("limit") || "30";

    const backendUrl = new URL(`${getBackendApiBase()}/api/chat/history`);
    ...
    console.error("Chat history proxy failed", error);
