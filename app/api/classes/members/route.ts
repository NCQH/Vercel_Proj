...
export async function GET(request: Request) {
  try {
    ...
    const backendUrl = new URL(`${getBackendApiBase()}/api/classes/members`);
    ...
    console.error("Class members proxy failed", error);
    return NextResponse.json({ error: "Failed to load class members" }, { status: 500 });
  }
}
