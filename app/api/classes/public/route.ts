...
export async function GET() {
  try {
    ...
    const backendUrl = new URL(`${getBackendApiBase()}/api/classes/public`);
    ...
    console.error("Public classes proxy failed", error);
    return NextResponse.json({ error: "Failed to load public classes" }, { status: 500 });
  }
}
